...
function getData() {
  ...
  rohabimSleminAndSheeritWitnOutSherit =
    (rohabMida + kant) / (rohabBad - hibur);
  ...
  sherWithOutSherit = Math.round(
    (rohabimSleminAndSheeritWitnOutSherit - shelem) * (rohabBad - hibur)
  );
...
function sherit() {
  ...
  if (rohabMida > 0) {
    ...
    if (mySherit > 0) sher = sher + hibur;

    sherLitleSimtri = (sherWithOutSherit + hibur) / 2;
    sherBigSimtri = (sherWithOutSherit + rohabBad) / 2;
    sherEqulAll =
      (rohabBad * shelemWithOutSherit + sherWithOutSherit) /
      (shelemWithOutSherit + 1);
    ...
    document.getElementById("op3Shalem").innerHTML =
      rohabBad + " X " + (shelem - 1);
    document.getElementById("op3InL").innerHTML = sherBigSimtri;
    document.getElementById("op3InR").innerHTML = sherBigSimtri;

    document.getElementById("op4Shalem").innerHTML =
      sherEqulAll.toFixed(1) + " X " + (shelem + 1);
    document.getElementById("op4InL").innerHTML = "----";
    document.getElementById("op4InR").innerHTML = "----";

    document
      .getElementById("hashebSheeritBtn")
      .addEventListener("click", drawRealSizeSampleOneSide);
  } //end of "if"
  else alert("Enter width");
} //End of sheerit
...
function orehFun() {
  ...
  if (orehMida > 0) {
    ...
    if (orehDisplayStatus != "none") {
      ...
      if (orehMida > 0) {
        if (tfiraOption == "plata") oreh = orehMida * (tmpShalem + one);

        if (tfiraOption == "awings") {
          if (volan > 0) {
            if (document.getElementById("sugGali").value == "kipul")
              oreh =
                (orehMida + volan + kipulHebel + hebel * 3) * (tmpShalem + one);
            else oreh = (orehMida + volan + hebel * 3) * (tmpShalem + one);
          } else oreh = (orehMida + hebel * 2) * (tmpShalem + one);
        }
        ...
        if (tfiraOption == "nofeletWithVolan") {
          if (document.getElementById("sugGali").value == "kipul")
            oreh =
              (orehMida + hebel * 2 + sharvul + kipulHebel + volan) *
              (tmpShalem + one);
          else
...
function saveOrehBad() {
  ...
  document.getElementById("orehGalil").value = (
    Number(document.getElementById("orehGalil").value) -
    totalOreh / 1000
  ).toFixed(2);
  ...
  if (loadFromLocal == null) {
    ...
    loadFromLocal.length =
      Number(loadFromLocal.length) - (saveData / 1000).toFixed(2);
    ...
    if (badArr == null) {
      ...
      for (i = 0; i < badArr.length; i++) {
        if (loadFromLocal.name == badArr[i].name) {
          alert("Save in local");
          badArr[i].length =
            Number(badArr[i].length) - (saveData / 1000).toFixed(2);
...
function deleteLastOreh() {
  var lastNode = document.getElementById("listOreh");
...
function drawRealSize(event) {
  ...
  if (opthioGzira == "tr1" || opthioGzira == "tr3" || opthioGzira == "tr4") {
    sizePice = shelem + 1;
    if (mySherit > 0) sizePice = shelem + 2;
  }
  if (opthioGzira == "tr2") {
    sizePice = shelem + 2;
  }
  ...
  for (var i = 0; i < sizePice; i++) {
    ...
    else mainDiv.style.justifyContent = "center";
  } // end for
} //end of drawRealSize()
...
function drawRealSizeSample(e) {
  ...
  for (var i = 0; i < 3; i++) {
    ...
    if (i == 0) {
      if (opthioGzira == "op1Btn") nodeChild.style.width = w + "%";
    ...
    if (opthioGzira == "op1Btn") mainDiv.style.justifyContent = "start";
    else mainDiv.style.justifyContent = "center";
  } // end for
} //end of drawRealSize()
...
function drawRealSizeSampleOneSide(event) {
  ...
  for (var i = 0; i < shelem + 1; i++) {
    ...
    if (i < shelem) nodeChild.style.width = w + "%";

    if (i == shelem) nodeChild.style.width = w * (sher / rohabBad) + "%";
...
function showBadRecord() {
  if (window.localStorage == null) {
    ...
    badList.innerHTML =
      "<tr><th>#</th><th>Name</th><th>Width</><th>oreh</th></tr>";
    ...
    var trBadList = document
      .getElementById("badListTable")
      .querySelectorAll("tr");

    for (var i = 1; i < trBadList.length; i++) {
...
function selectBad() {
  document.getElementById("badName").textContent =
    document.getElementById("enterNameBad").value;
  document.getElementById("rohabBad").value =
    document.getElementById("enterWidthBad").value;
  document.getElementById("orehGalil").value =
    document.getElementById("enterOrehBad").value;
...
function deleteBadItem() {
  ...
  if (loadFromLocal == null) {
    ...
    for (var i = 0; i < loadFromLocal.length; i++) {
      if (
        loadFromLocal[i].name == document.getElementById("enterNameBad").value
      ) {
        if (
          confirm(
            " Delete " + document.getElementById("enterNameBad").value + " ?"
          )
        ) {
...
function showClientRecord() {
  if (window.localStorage == null) {
    ...
    var clientListData = JSON.parse(
      localStorage.getItem("ClientListLocalData")
    );
    ...
    if (clientListData == null) {
      ...
      clientList.innerHTML =
        "<tr><th>#</th><th>Name</th><th>Tel</><th>Adress</th></tr>";
...
function addClient() {
  ...
  if (clientArray == null) {
    ...
    for (var i = 0; i < clientArray.length; i++) {
      if (
        clientArray[i].clientName ==
        document.getElementById("enterclientName").value
      ) {
        iHave = false;
        (clientArray[i].clientTel = document.getElementById("enterTel").value),
          (clientArray[i].clientAdress =
            document.getElementById("enterStreet").value);
        localStorage.setItem(
          "ClientListLocalData",
          JSON.stringify(clientArray)
        );
        alert("Item Updated");
      }
    }
...
function selectClient() {
  document.getElementById("clientNameLablel").textContent =
    document.getElementById("enterclientName").value;
...
function saveOrderData() {
  if (confirm(" SAVE DATA ?")) {
    ...
    if (orderRecordArr == null) {
      ...
      localStorage.setItem(
        "orderRecordStorage",
        JSON.stringify(fristOrderData)
      );
    } else {
      orderRecordArr.push(orderData);
      localStorage.setItem(
        "orderRecordStorage",
        JSON.stringify(orderRecordArr)
      );
    }
  }
}
...
document.getElementById("hashebSheeritBtn").addEventListener("click", sherit);

document.getElementById("tr1").addEventListener("click", drawRealSize);
document.getElementById("tr2").addEventListener("click", drawRealSize);
document.getElementById("tr3").addEventListener("click", drawRealSize);
document.getElementById("tr4").addEventListener("click", drawRealSize);

document
  .getElementById("exitMenuBadBtn")
  .addEventListener("click", function () {
    document.getElementById("badSec").style.display = "none";
  });

document.getElementById("badName").addEventListener("click", function () {
...
document
  .getElementById("saveInLocalStorage")
  .addEventListener("click", enterBadRecord);
document
  .getElementById("deleteInLocalStorage")
  .addEventListener("click", deleteBadItem);
document.getElementById("selectBad").addEventListener("click", selectBad);

document
  .getElementById("clientNameLablel")
  .addEventListener("click", function () {
...
document
  .getElementById("clientNameLablel")
  .addEventListener("click", showClientRecord);

document.getElementById("exitClientBtn").addEventListener("click", function () {
  document.getElementById("clientSec").style.display = "none";
});

document
  .getElementById("deleteDataLocal")
  .addEventListener("click", deleteClient);

document.getElementById("saveDataLocal").addEventListener("click", addClient);
document.getElementById("selectClient").addEventListener("click", selectClient);

document.getElementById("saveDataBtn").addEventListener("click", saveOrderData);

document
  .getElementById("exitOpthionData")
  .addEventListener("click", function () {
...
document.getElementById("opthionData").addEventListener("blur", function () {
  alert("fo");
  document.getElementById("opthionData").style.display = "none";
});

document
  .getElementById("showSavedData")
  .addEventListener("click", showOrderRecord);

document
  .getElementById("deleteLastOrderBtn")
  .addEventListener("click", deleteLastOrder);
//document.getElementById("mySherit").addEventListener("click", loadPices);
loadPices();

document
  .querySelector("#sheeritSection .exit")
  .addEventListener("click", function () {
    document.querySelector("#sheeritSection").style.display = "none";
  });

document
  .querySelector("#orehTable .exit")
  .addEventListener("click", function () {
    document.querySelector("#orehTable").style.display = "none";
  });
